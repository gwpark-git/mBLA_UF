...
int_v1 = interp1d(dat_v1[:,0], dat_v1[:,1])
int_v2 = interp1d(dat_v2[:,0], dat_v2[:,1])
...
int_v1_ref = int_v1(z_ref)
int_v2_ref = int_v2(z_ref)
int_test_Ny200_ref = int_test_Ny200(z_ref)
int_test_Ny100_ref = int_test_Ny100(z_ref)


dat_test_now = loadtxt('../scripts/test.dat')
int_test_now = interp1d(dat_test_now[:,0], dat_test_now[:,1])
int_test_now_ref = int_test_now(z_ref)
...
plt.plot(z_ref/L_channel, int_v1_ref, 'b-', label = 'mBLA Python code v1.0 (used in manuscript)')
plt.plot(z_ref/L_channel, int_v2_ref, 'r-', label = 'mBLA Python code v2.0')
plt.plot(z_ref/L_channel, int_f_ref, 'k.', label = 'mBLA C++ code (not published yet, used FDM for ODE)')
...
plt.plot(z_ref/L_channel, int_test_now_ref, 'co', label = 'new test')
...
plt.subplot(212)
plt.plot(z_ref/L_channel, (dat_ref_FEM[ind_FEM, 1] - int_v1_ref), 'b-', label = 'mBLA Python code v1.0 (used in manuscript)')
plt.plot(z_ref/L_channel, (dat_ref_FEM[ind_FEM, 1] - int_v2_ref), 'r-', label = 'mBLA Python code v2.0')
...
plt.plot(z_ref/L_channel, (dat_ref_FEM[ind_FEM, 1] - int_test_now_ref), 'co', label = 'new test')
