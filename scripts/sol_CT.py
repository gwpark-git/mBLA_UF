...
def get_cond(cond_PS, phi_bulk, a_colloid, a_hydrodynamic, Va, kT, dz, Gk):
    """ Update condition dictionary inherited from cond_PS (defined by get_cond in sol_solvent.py)
        ...
        This CT0 case gives the explicit solution of phi_w(z) in accordance with Eq. (52) in [1].
        Therefore, CT0 DO NOT require the FPI iterator.
        In the general CT, however, the particle-contributed osmotic pressure Pi(phi_w(z)) make Eq. (52) in [1] as the implicit equation.
        ...
        which, however, the iterator is not included. Such FPI iterator (Eq. (D4) [1]) is defined on sol_GT.py.
    ...
    re['Pe_R'] = re['R']*re['vw0']/re['D0']           # radial Peclet number from Eq. (37) in [1]
    re['epsilon_d'] = 1./re['Pe_R']                   # selected perturbation constant using Eq. (41) in [1]
...
def get_gpm(pm, z_div_L, dz_div_L, Pi_div_DLP_arr, k):
    """ [Overhead version] Using expresion for gpm(zt=1) in Eq. (46) [1]
...
def gen_gpm_arr(pm, z_div_L_arr, Pi_div_DLP_arr, k, gpm_arr):
    """ Generation gpm_arr based on Eq. (46) [1]
...
def get_Gk(k, dz_div_L, Pi_div_DLP_arr, denom_Gk_BC_specific):
    """ [Overhead version] Using expression "Gk = -(Bp - Ap) = Bm - Am" in Eq. (46) [1]
    ...
    Note that denom_Gk_BC_specific  = 2 sinh(k) in [1] for the case of pressure-inlet-boundary condition.
    When the velocity-inlet-boundary condition is used, denom_Gk_BC_specific = 2 cosh(k) (see [2] for details). 
...
def get_P(r_div_R, z_div_L, Pper_div_DLP, k, Bp, Bm, gp, gm):
    """ Pout expression in Eq. (45) in [1]
    because P = Pout in Eq. (49) in [1]
    """
    return Pper_div_DLP\
        + exp(k*z_div_L)*(Bp + gm)\
        + exp(-k*z_div_L)*(Bm + gp)
...
def get_u(r_div_R, z_div_L, k, Bp, Bm, gp, gm, lam1):
    """ Using u^out expression in Eq. (45) in [1]
    because matched asymptotic u in Eq. (49) in [1] with constant transport properties
    give the same expression as u^out in Eq. (45) in [1]
    """
...
def get_v(r_div_R, z_div_L, Pi_div_DLP, k, alpha_ast, Bp, Bm, gp, gm, membrane_geometry):
    """ Using v^out expression in Eq. (45) in [1]
    One must careful about coordinate function r_div_R in comparison with y_div_R
    because the direction is reversed, which means the minus sign in Eq. (45) in [1] for v^out
    should be reversed to plus sign since we use r_div_R in this code.
    This will be clear if we compare v^out(y,z) in Eq. (45) in [1] with v(r,z) in Eq. (31) in [1] for pure solvent flow
...
def get_phi(r_div_R, z_div_L, vw_div_vw0, epsilon_d, phiw, phi_bulk):
    """ matched asymptotic phi for constant transport properties (CT) using Eqs. (43) and (49) in [1]
