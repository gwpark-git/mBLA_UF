...
def get_cond(pre_cond, Pin, Pout, Pper): # conditions for pure solvent flow
    ...
    DTP_PS = DTP_HP*2.*tanh(k/2.)/k     # Length-averaged TMP for pure solvent flow in Eq. (7)
    ...
    # print ('Pin, Pout, Pper in Pa : ', int(Pin), int(Pout), int(Pper))
    # print ('DLP, DTP, DTP_HP in Pa : ', int(DLP), int(DTP), int(DTP_HP))
